...
| `OPENCODE_EXPERIMENTAL_LSP_TY`                  | boolean | Enable ty LSP server for Python              |
| `OPENCODE_EXPERIMENTAL_PLAN_MODE`               | boolean | Enable experimental plan mode                |
| `OPENCODE_EXPERIMENTAL_CHAT_MAX_RETRIES`        | number  | Number of retries for chat completions       |
| `OPENCODE_EXPERIMENTAL_DISABLE_PASTE_SUMMARY`   | boolean | Disable paste summary                        |
| `OPENCODE_EXPERIMENTAL_BATCH_TOOL`              | boolean | Enable the batch tool                        |
| `OPENCODE_EXPERIMENTAL_OPEN_TELEMETRY`          | boolean | Enable OpenTelemetry spans for AI SDK        |
| `OPENCODE_EXPERIMENTAL_CONTINUE_LOOP_ON_DENY`   | boolean | Continue agent loop when tool call is denied |
| `OPENCODE_EXPERIMENTAL_MCP_TIMEOUT`             | number  | Timeout in ms for MCP requests               |

All experimental environment variables can also be configured via the `experimental` key in your config file. See [experimental config options](/docs/config/#experimental).
